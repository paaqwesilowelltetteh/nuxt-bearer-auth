...
26. Phase 4 ships `<Can>`/`<Cannot>` on one shared pure client evaluator (`evaluateAbilities`) mirroring the frozen matcher semantics. An empty requirement means unrestricted, and disabled authorization renders as cannot — both identical to existing route/server behavior; there is no special UI mode for `enabled: false`.
27. Authorization directives (`v-can`) are rejected: they duplicate the component and composable APIs, weaken TypeScript ergonomics, complicate SSR/hydration, and add public surface without new capability.
28. Delivering the `@nuxt/schema` runtime-config and h3 event-context type augmentations to consumers is deferred to a separate maintenance change: fixing it safely requires declaration-graph/build work (unbuild rollup fragility, decision 24), no runtime behavior depends on it, and Phase 4 scope forbids build-system restructuring.
